...
def email_user(email, message, account_name):
    ...
    iam_client = boto3.client('iam')
    print(email)
    email_content = (iam_client.list_user_tags(UserName=email))
    email_tags = email_content['Tags']
    email_tags_dict=email_tags[0]
    to_email = email_tags_dict['Value']
